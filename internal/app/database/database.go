...
func connect() {
	connectOnce.Do(func() {
		...
		// Create db instance
		host := os.Getenv("LDSMEDIA_DB_READ_HOST")
		username := os.Getenv("LDSMEDIA_DB_USERNAME")
		password := os.Getenv("LDSMEDIA_DB_PASSWORD")
		dbName := os.Getenv("LDSMEDIA_DB_NAME")
		dbPort := os.Getenv("LDSMEDIA_DB_PORT")

		if dbPort == "" {
			dbPort = "3306"
		}
		if host == "" || username == "" || password == "" || host == "" {
			log.Fatal("invalid db config : env variables not set [LDSMEDIA_DB_READ_HOST] [LDSMEDIA_DB_USERNAME] [LDSMEDIA_DB_PASSWORD] [LDSMEDIA_DB_NAME]")
		}
...
func Queryx(query string, args ...interface{}) (*sqlx.Rows, error) {
	connect()
	return currentInstance.read.Queryx(query, args...)
}
