...
func login(req echo.Context) error {
	...
	if username == "" || password == "" {
		...
		return req.String(http.StatusBadRequest, "Invalid username/password supplied")
	}
	// check creds from db
	user, err := dbsiteuser.ReadActiveByUsername(username)
...
func CheckIfUsernameExists(username string) (bool, error) {
	exists := false
	existingUser, err := dbsiteuser.ReadByUsername(username)
	if err != nil {
		return false, err
	}
